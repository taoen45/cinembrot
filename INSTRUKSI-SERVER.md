...
| 2026-09-17 | **Default Filter Tahun 2026 & Urutan Rilis Terbaru di Seluruh Halaman Katalog:** (1) Backend handler publik di `server/handlers.go` (`HandleAnime`, `HandleHollywood`, `HandleDramaPendek`, dan `HandleFilter`) kini menetapkan filter default tahun sekarang (`2026`) dan urutan rilis terbaru (`year_desc`) saat pertama kali halaman dimuat tanpa query param, dengan fleksibilitas opsi `all` saat user sengaja memilih "Semua Tahun". (2) Endpoint backend `POST /api/movies/filter` diperbarui untuk mendukung pemfilteran semua tahun (`year=all`). (3) Seluruh template katalog publik (`server/views/anime.html`, `server/views/hollywood.html`, `server/views/drama_pendek.html`, dan `server/views/list.html`) otomatis me-render opsi dropdown tahun terpilih pada `2026` dan urutan pada `Rilis Terbaru`. Tombol reset filter otomatis mengembalikan state ke default 2026 & rilis terbaru secara instan via AJAX tanpa reload. |
| 2026-09-17 | **Penghapusan Form Pencarian Beranda, Penambahan Banner Adsterra (In-Feed & Smartlink VIP) & Section Jelajahi Genre Populer:** (1) Menghapus form pencarian dan live filter cepat dari halaman beranda (`server/views/home.html`) karena fungsi pencarian telah tersedia di header navbar dan halaman filter lengkap (`/filter`). (2) Menggantinya dengan section "Jelajahi Berdasarkan Kategori & Genre" yang menampilkan kartu visual interaktif genre film (Aksi, Petualangan, Animasi, Komedi, Drama, Horor, Romantis, Sci-Fi, Misteri, Thriller, Kejahatan) lengkap dengan shortcut ke filter lengkap. (3) Menambahkan slot banner iklan Adsterra strategis di tengah konten beranda (*mid-home*): In-Feed Native Banner (`AdsterraNative`), Responsive Display Banner (`AdsterraBanner728` & `AdsterraBanner300`), dan Adsterra Smartlink VIP Stream Promo Card untuk memaksimalkan eCPM dan konversi pendapatan ads situs. (4) Merapikan controller JavaScript di `home.html` menjadi controller paginasi film rilis terbaru murni tanpa dependensi form pencarian lama. |
| 2026-09-17 | **Live Filter AJAX & Optimasi Beban Pencarian Minimal 3 Karakter pada Halaman Filter Lengkap (/filter):** (1) Implementasi controller AJAX POST penuh pada halaman filter lengkap (`server/views/list.html`) dengan instant auto-trigger pada seluruh dropdown (jenis film, tahun, genre, negara, rating, urutan), status Live Filter badge, indikator loading halus, paginasi dinamis tanpa reload, dan sinkronisasi URL address bar via `window.history.pushState`. (2) Optimasi beban server & database: pencarian kata kunci menerapkan debounce 400ms, proteksi minimal 3 karakter (baik di client-side maupun backend `HandleFilter` & `HandleAPIMoviesFilter` di `server/handlers.go`) untuk mencegah query wildcard LIKE 1-2 karakter yang berat di MariaDB, serta `AbortController` untuk membatalkan request lama yang belum selesai saat ada ketikan baru. (3) Tombol reset filter otomatis mengembalikan seluruh opsi ke default 2026 dan rilis terbaru secara instan via AJAX. |
| 2026-09-17 | **Perbaikan Bug Render Tampilan Filter Lengkap (/filter):** Memperbaiki syntax perbandingan template di `server/views/list.html` pada dropdown Tahun Rilis dari `{{if eq $.CurrentYear "all"}}` menjadi `{{if le $.CurrentYear 0}}`. Perbandingan integer dengan string pada engine Go template sebelumnya memicu error *incompatible types for comparison* yang menyebabkan proses render HTML terputus di tengah form, sehingga dropdown genre, negara, rating, sorting, serta movie grid tidak tampil. Setelah perbaikan, seluruh form filter dan grid film di `/filter` ter-render 100% lengkap dan sempurna. |
| 2026-09-17 | **Perbaikan Total Video Streaming Player & Pembaruan Server Embed Aktif (VidLink HD, VidSrc Pro, VidSrc Mirror):** (1) Investigasi mendalam menemukan akar masalah pemutaran video gagal: Server 1 (`vidsrc.xyz`) dan Server 2 (`player.autoembed.cc`) yang sebelumnya tersimpan pada 31.000+ data di tabel `stream_links` mengalami DNS lookup failure / domain mati permanen, sehingga pemutar default langsung error/layar hitam saat dibuka penonton. (2) Mengganti daftar penyedia embed di `provider/embed/embed.go` dengan server video aktif berkecepatan tinggi: Server 1 (VidLink HD - Next.js super cepat, 1080p, minim iklan), Server 2 (VidSrc Pro - vidsrc.to resmi dan stabil), Server 3 (VidSrc Mirror - vidsrc.pm cadangan resmi), Server 4 (2Embed VIP), dan Server 5 (MultiEmbed). (3) Menambahkan dukungan pemutaran berbasis IMDb ID (`tt...`) sebagai fallback jika film/anime tidak memiliki TMDb ID. (4) Smart Auto-Healing di backend `server/handlers.go` (`HandleMovieDetail`): otomatis mendeteksi link mati dan meregenerasi server aktif baru tanpa perlu mengubah skema/data database massal secara manual. (5) Perbaikan endpoint `/api/movie/{id}/refresh-stream` dan sinkronisasi JavaScript `buildEpisodeEmbedURL()` di `server/views/detail.html` agar perpindahan server dan episode anime/drama berjalan mulus tanpa reload. |
