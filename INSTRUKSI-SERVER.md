...
| 2026-09-16 | **Arsitektur SEO Terbaik & Fast-Indexing Mesin Pencari (Google, Bing, Yandex):** (1) Modul baru `server/seo.go` yang menghasilkan Dynamic XML Sitemap resmi (`GET /sitemap.xml`) berstandar protokol Sitemaps.org 0.9 beserta ekstensi Google Image (`xmlns:image`) yang memuat seluruh ribuan katalog film, anime, dan serial drama dengan atribut `<lastmod>`, `<changefreq>`, dan poster `<image:image>`. (2) Endpoint standar crawler `GET /robots.txt` dengan directive ramah bot perayap dan tautan resmi ke sitemap. (3) Pemasangan meta tag SEO komprehensif di `<head>` (`layout.html`): dynamic meta description, dynamic meta keywords kaya kata kunci target penonton, canonical URL, hreflang alternates, OpenGraph & Twitter Cards preview share medsos, serta meta robots `index, follow, max-image-preview:large, max-snippet:-1`. (4) Structured Data Schema.org (JSON-LD) format `Movie` dan `TVSeries` di halaman detail lengkap dengan `AggregateRating` (agar bintang kuning rating muncul di hasil pencarian Google) serta `WebSite` dengan `SearchAction` (Sitelinks Searchbox) di beranda. (5) Penambahan kontrol SEO di CMS Admin `/admin/settings` (`server/views/admin_settings.html` & `server/admin_settings_handlers.go`) untuk pengaturan domain publik (`site_url`), meta default, dan kode verifikasi Webmaster (Google Search Console, Bing Webmaster, Yandex). |
