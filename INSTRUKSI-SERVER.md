...
| 2026-09-16 | **Fitur Tombol Refresh Server Streaming Mandiri & Aturan Cooldown 24 Jam (Stream & Download):** (1) Penambahan tombol interaktif "Cek / Segarkan Server" di bilah player video `server/views/detail.html`. (2) Endpoint backend baru `POST /api/movie/{id}/refresh-stream` di `server/handlers.go` dan `server/server.go` untuk me-rescrape server embed video secara mandiri dari TMDb jika player rusak/broken. (3) Penerapan aturan penonaktifan tombol (*disabled cooldown*) selama 1 hari (24 jam) persisten di browser (`localStorage`) jika hasil URL dari sumber sama persis, berlaku serentak untuk tombol refresh streaming dan tombol refresh download link. |
