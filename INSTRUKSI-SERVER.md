...
| 2026-09-16 | **Peningkatan Scraper Cerdas: Auto-Discovery & Scraping Seluruh Halaman (Anime, Drama & Hollywood):** (1) Scraper kini otomatis memeriksa total halaman (`total_pages` / `last_visible_page`) yang tersedia dari API penyedia (TMDb / MyAnimeList Jikan) tanpa mewajibkan pengguna mengetik argumen halaman. Cukup jalankan `.\cinembrot.exe -scrape-hollywood -year 2026`, `.\cinembrot.exe -scrape-anime -year 2026`, atau `.\cinembrot.exe -scrape-drama -year 2026`, sistem otomatis menelusuri seluruh halaman dari halaman 1 hingga selesai. (2) Menambahkan dukungan flag `-anime-pages` (dan `-anime-limit`) jika ingin membatasi jumlah halaman secara manual. (3) Perbaruan panduan CLI dan dokumentasi di `README.md` dan `main.go`. |
